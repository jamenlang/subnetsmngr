    ...
    //make these sticky for next import
		...
		if (!ip_in_network($ip, $_POST['subnet_addr']))
			...
			if (pg_num_rows($result) == 1) {
				if (isset($_POST['new'])){
					$_SESSION['notice_msg'] .= 'Host ' . $ip . ' already exists, updating if necessary.';
					$sql = "update hosts set free = '" . (isset($_POST['used']) ? '0' : '1') . "', description = '{$description}', notes = '{$notes}', last_updated = now(), last_updated_user_id = '{$_SESSION['user']['id']}', gateway = '" . (isset($_POST['gateway']) && $_POST['gateway'] ? 1 : 0) . "' where addr = '{$ip}'";
				}
			} else
				$sql = "insert into hosts (subnet_id, addr, free, gateway, description, notes, last_updated, last_updated_user_id) values('{$_GET['subnet_id']}', '{$ip}', '" . (isset($_POST['used']) ? '0' : '1')  . "', '" . (isset($_POST['gateway']) ? 1 : 0) . "', '{$description}', '{$notes}', now(), '{$_SESSION['user']['id']}')";
...
//print_r($parent_subnet_info['notes']);
